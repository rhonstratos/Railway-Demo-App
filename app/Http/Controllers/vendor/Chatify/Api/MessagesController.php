...
use Chatify\Http\Controllers\Api\MessagesController as Controller;
...
class MessagesController extends Controller
{
	...
	public function idFetchData(Request $request)
	{
		...
		if ($request['type'] == 'user') {
			...
			if ($fetch) {
				$userAvatar = !is_null($fetch->accountSettings->profile_img)
					? asset("/storage/users/{$fetch->userId}/images/profile/{$fetch->accountSettings->profile_img}")
	...
	public function getContacts(Request $request)
	{
		...
		$users = Message::join('users', function ($join) {
			...
			// ->join('account_settings', 'account_settings.user_id', '=', 'users.id')
			->where(function ($q) {
			    $q->where('ch_messages.from_id', Auth::user()->id)
			    	->orWhere('ch_messages.to_id', Auth::user()->id);
		    })
			->where('users.id', '!=', Auth::user()->id)
			->select('users.*', /* 'account_settings.profile_img', */ DB::raw('MAX(ch_messages.created_at) max_created_at'))
			->orderBy('max_created_at', 'desc')
			->groupBy('users.id')
			// ->groupBy('users.userId')
			// ->groupBy('users.firstname')
			// ->groupBy('users.lastname')
			// ->groupBy('users.contact')
			// ->groupBy('users.birthday')
			// ->groupBy('users.two_factor_secret')
			// ->groupBy('users.is_2fa_enabled')
			// ->groupBy('users.is_business')
			// ->groupBy('users.email')
			...
			// ->groupBy('users.google2fa_secret')
			// ->groupBy('users.google2fa_ts')
			// ->groupBy('users.is_banned')
			// ->groupBy('users.is_banned_reason')
			// ->groupBy('account_settings.id')
	...
	public function search(Request $request)
	{
		...
		$is_biz = Auth::user()->is_business;
		// $query = User::select('id', 'firstname', 'lastname', 'is_business')->get()
		// 	->filter(function ($v, $k) use ($input, $is_biz) {
		// 		return ($v->is_business != $is_biz &&
		// 			Str::contains(
		// 				strtolower($v->firstname . ' ' . $v->lastname),
		// 				strtolower($input)
		// 			))
		// 			? true
		// 			: false;
		...
		try {
			...
			// 	$records = $records->where('id', $item['id']);
			// }
			// dd($records->get());

			$records = $records->where('is_business', '!=', $is_biz);
			$records = $records->where('firstname', 'like', '%' . $input . '%');
			$records = $records->orWhere('lastname', 'like', '%' . $input . '%');
		...
		} catch (\ErrorException | \BadMethodCallException $err) {
			$getRecords = '<p class="message-hint center-el"><span>Nothing to show.</span></p>';
		}
	}
