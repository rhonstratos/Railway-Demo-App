	...
	if (messenger != 0) {
		...
		$.ajax({
			...
			success: (data) => {
				...
				messageInput.focus();
				// update info in view
				$(".messenger-infoView .info-name").html(data.fetch.firstname + ' ' + data.fetch.lastname);
				$(".m-header-messaging .user-name").html(data.fetch.firstname + ' ' + data.fetch.lastname);
				// Star status
				data.favorite > 0
...
function updateContactItem(user_id) {
	if (user_id != auth_id) {
		...
		$.ajax({
			...
			method: "POST",
			data: {
				_token: decodeURIComponent(access_token),
				user_id,
			},
			dataType: "JSON",
			success: (data) => {
