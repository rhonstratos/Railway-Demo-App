	...
	@if ($from_id != $to_id)
		<div class="message-card" data-id="{{ $id }}">
			...
			@if (@$attachment[2] == 'image')
				<div class="image-file chat-image" style="width: 250px; height: 150px;background-image: url('{{ 'https://www.' . config('app.url') . '/storage/' . config('chatify.attachments.folder') . '/' . $attachment[0] }}')">
				</div>
			@endif
		</div>
	@endif
...
@if ($viewType == 'sender')
	<div class="message-card mc-sender" title="{{ $fullTime }}" data-id="{{ $id }}">
		...
		{{-- If attachment is an image --}}
		@if (@$attachment[2] == 'image')
			<div class="image-file chat-image" style="margin-top:10px;width: 250px; height: 150px;background-image: url('{{ 'https://www.' . config('app.url') . '/storage/' . config('chatify.attachments.folder') . '/' . $attachment[0] }}')">
			</div>
		@endif
	</div>
@endif
