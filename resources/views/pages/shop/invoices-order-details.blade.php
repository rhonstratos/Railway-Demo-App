		...
		<div class="business-header sm:flex-row sm:justify-between sm:items-center">
			<span class="w-fit flex flex-row gap-3 items-center cursor-pointer" onclick="location.href='{{ URL::previous() }}'">
				...
				<div class="flex flex-col gap-1">
					...
					<span class="italic text-[12px]">Order id: {{ $order->orderId }}</span>
				</div>
			</span>
			@if(false)
		...
		<div class="h-[calc(100vh_-_183px)] sm:h-[calc(100vh_-_137px)] lg:h-[calc(100vh_-_102px)] xl:h-[calc(100vh_-_94px)] px-4 pb-4 flex flex-col gap-2 overflow-y-auto">
			<div class="business-whitecard-bg">
				...
				<div class="flex flex-col gap-2">
					...
					<section class="md:px-2 flex gap-2 flex-col">
						<h2 class="text-darkblue text-[16px] xl:text-[20px] font-extrabold">
							Customer
						</h2>
						<div class="px-2 flex flex-col gap-2">
							<span>Name: {{ $order->user->firstname . ' ' . $order->user->lastname }}</span>
							<span>Email: {{ $order->user->email }}</span>
							<span>Phone Number: {{ $order->user->contact }}</span>
							<span>Alternate Phone Number: {{ $order->alt_contact ?? __('Undefined') }}</span>
						</div>
					</section>
					@if (false)
				...
				<section class="mt-[20px]">
					...
					<div class="h-auto mb-2 p-2 flex flex-row bg-dirtywhite font-semibold">
						<span class="basis-1/4 truncate">Item</span>
						<span class="basis-1/4 text-left truncate">Quantity</span>
						<span class="basis-1/4 text-left truncate">Price</span>
						<span class="basis-1/4 text-end truncate">Subtotal</span>
					</div>

					<!-- item, quantity, price and subtotal -->
					<ul class="flex flex-col">
						<li class="flex gap-2 flex-col px-2">
							<div id="billing-items">
								@foreach ($order->items as $ordered)
									<div class="flex flex-row gap-2">
										<Span class="basis-1/4 capitalize truncate">{{ $ordered->product->name }}</Span>
										<span class="basis-1/4 text-left truncate">{{ $ordered->quantity }}</span>
										<span class="basis-1/4 text-left truncate">Php {{ number_format((float) $ordered->product->price, 2, '.', ',') }}</span>
										<span class="basis-1/4 text-end truncate">Php {{ number_format((float) $ordered->quantity * (float) $ordered->product->price, 2, '.', ',') }}</span>
									</div>
								@endforeach
							</div>
				...
				<section class="pb-[20px] sm:pb-0">
					...
					<div class="flex flex-col gap-2">
						...
						<div class="flex gap-1 flex-row justify-end items-center text-[#5F6368] text-right font-extrabold">
							...
							<span class="basis-1/2 sm:basis-1/4" id="total">PHP {{ number_format((float) $order->total, 2, '.', ',') }}</span>
						</div>
					</div>
				</section>
			</div>
