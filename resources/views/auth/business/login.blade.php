...
mx-auto p-0">
    ...
    xl:p-10
    p-5">
            <form action="{{ route('auth.business.login') }}" method="post" autocomplete="on" aria-autocomplete="both" class="w-full">
                @csrf
                <!-- Session Status -->
				...
				@if (Route::has('auth.customer.login'))
					<div class="p-2 mb-5 text-white font-bold">
						<button data-href="{{ route('auth.customer.login') }}" onclick="location.href=this.dataset.href"
							...
							Not a shop owner? Login Here
						</button>
					</div>
				@endif
                @if (Route::has('auth.business.password.request'))
                    <div class="text-white text-center">
                        <a class="text-{{$site_settings->site_color_theme}} hover:text-red-500" href="{{route('auth.business.password.request')}}">Forgot Password?</a>
                    </div>
                @endif
            </form>
        </div>
        @if (Route::has('auth.business.register'))
            <div class="mt-4 text-center">
                Don't have an account yet? <a class="text-{{$site_settings->site_color_theme}} hover:text-red-500" href="{{route('auth.business.register')}}">Register Here</a>
            </div>
        @endif
    </div>
@endsection
